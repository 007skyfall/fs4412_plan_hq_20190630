...
unsigned int s5pc210_cpu_id= SMDK4412_ID;


extern u32 sclk_mmc4;
//u32 sclk_mmc4 = 0x160000000;
...
static int mshci_change_clock(struct mshci_host *host, uint clock)
{
	...
	else
	{
		...
		emmcdbg("EMMC CLOCK OUTPUT:: %dKHz -[div:%d]\n", emmc_clock_output/1000,div);
	...
	do {
		...
		{
			emmcdbg("there is problem of changing clock 0x%x",mshci_readl(host, MSHCI_CMD));
			return -1;
