...
unsigned int OmPin=BOOT_EMMC441;
...
int init_raw_area_table (block_dev_desc_t * dev_desc)
{
	...
	if (raw_area_control.magic_number != MAGIC_NUMBER_MOVI) {
		...
		raw_area_control.start_blk = RAW_AREA_SIZE/MOVI_BLKSIZE;
		...
		/* image 5 should be devicetree */
		image[5].start_blk = image[4].start_blk + image[4].used_blk;
		image[5].used_blk = MOVI_DTB_BLKCNT;
		image[5].size = PART_SIZE_DTB;
		image[5].attribute = 0x30;
		strcpy(image[5].description, "devicetree");
		dbg("Dtb: %d\n", image[5].start_blk);
...
//	init_raw_area_table();
	...
	if (attribute == 0x222) {
		...
		start_blk =0;// image[i].start_blk;
		blkcnt = image[i].used_blk;
		printf("%s bootloader.. %ld, %ld ", rw ? "writing":"reading",
				start_blk, blkcnt);
		sprintf(run_cmd,"mmc %s 0 0x%lx 0x%lx 0x%lx", //0-->1
				rw ? "write":"read",
				addr, start_blk, blkcnt);
	...
	if (attribute == 0x30) {
		...
		start_blk = image[i].start_blk;
		blkcnt = image[i].used_blk;
		printf("%s devicetree.. %ld, %ld ", rw ? "writing" : "reading",
				start_blk, blkcnt);
		if (1 == fuse_by_fastboot)
			sprintf(run_cmd, "mmc %s %s 0x%lx 0x%lx 0x%lx",
					rw ? "write" : "read", dev_number_write ? "1" : "0",
